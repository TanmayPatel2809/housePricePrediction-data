...
from housePricePrediction.utils.utils import get_logger, run_shell_command
...
def initialize_dvc_storage(dvc_remote_name: str, dvc_remote_url: str) -> None:
    if not run_shell_command("dvc remote list"):
        DATA_UTILS_LOGGER.info("Initializing DVC remote storage...")
        run_shell_command(f"dvc remote add -d {dvc_remote_name} {dvc_remote_url}")
        run_shell_command(f"git add .dvc/config")
        run_shell_command(f"git commit -nm 'Configure DVC remote storage'")
    else:
        DATA_UTILS_LOGGER.info("DVC remote storage is already configured.")
